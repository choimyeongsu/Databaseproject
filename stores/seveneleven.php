...
<?php
                ...
                $query ="select * from db where brand='GS25' order by tag";
        ...
        <body>
          ...
          <div id="search_box">
            ...
            if(isset($_POST["submit"])){
                ...
                //데이터베이스에 쿼리 명령문을 보낸다
                $sth=$con->prepare("SELECT * FROM db WHERE name like '%$str%' and brand='SevenEleven'");
...
<?php
                $connect = mysqli_connect('localhost', 'root', 'rlaehddnd0422@', 'PROJECT') or die ("connect fail");
                $query ="select * from db where brand='seveneleven' order by tag";
                $result = $connect->query($query);
                $total = mysqli_num_rows($result);

        ?>
        <h1 align=center>SevenEleven 제품 정보 모음</h1>
        <table align = center>
        <thead align = "center">
        <tr style ="font-weight:bold;">
        <td width = "300" align = "center">상호명</td>
